...
`PortfolioPlanning` → `PortfolioEvents` → `PortfolioResults` → `PortfolioContact`.

## O que cada componente faz

| Componente | Seção (nav) | Estado atual |
| --- | --- | --- |
| `PortfolioNavbar.tsx` | Header | Menu com 8 itens: Início/Sobre mim/Serviços/Marcas atendidas/**Posts**/Eventos/Resultados/Contato (o item era "Planejamentos", renomeado numa sessão seguinte — ver "Decisões" abaixo; o `href`/`id` internos continuam `#planejamentos`, só o texto visível mudou). Destaca esse item em dourado quando a rota atual é `/portfolio/planejamentos/*`. Nav desktop só aparece em `xl:` (menu é longo); hambúrguer cobre até `xl`. Link "Ver pacotes" → `/`. |
